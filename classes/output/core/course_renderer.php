...
namespace theme_dai\output\core;
...
$PAGE->theme->settings->coursetilestyle = 1;
$PAGE->theme->settings->trimtitle = 100;
$PAGE->theme->settings->trimsummary = 200;
$PAGE->theme->settings->titletooltip =1;
/**
...
class course_renderer extends \theme_boost\output\core\course_renderer {
            ...
            $header = '
                <div id="category-course-list">
                    ...
                    <div class="class-list">
                        <h2  class="centerit">'.$newcourse . '</h2>
                    </div>';
            $content = '';
            $footer = '
                    </div>
            ...
            if (count($rcourseids) > 0) {
                foreach ($acourseids as $courseids) {
                    ...
                    foreach ($courseids as $courseid) {
                        ...
                        $trimtitle = format_string(theme_dai_course_trim_char($course->fullname, $trimtitlevalue));
                        $summary = theme_dai_strip_html_tags($course->summary);
                        $summary = format_text(theme_dai_course_trim_char($summary, $trimsummaryvalue));
						...
						if ($cat = coursecat::get($course->category, IGNORE_MISSING)) {
								...
								//$catcontent .= html_writer::link(new moodle_url('/course/index.php', array('categoryid' => $nfcategroyid)),	$cat->get_formatted_name(), array('class' => $cat->visible ? '' : 'dimmed'));
        ...
        protected function coursecat_category(coursecat_helper $chelper, $coursecat, $depth) {
            if (!theme_dai_get_setting('enablecategoryicon')) {
            ...
            $val = theme_dai_get_setting('catsicon');
        ...
        public function frontpage_my_courses() {
            ...
            $nomycourses = '<div class="alert alert-info alert-block">' . get_string('nomycourses', 'theme_dai') . '</div>';
            $lastaccess = '';
            $output = '';
            if (theme_dai_get_setting('frontpagemycoursessorting')) {
